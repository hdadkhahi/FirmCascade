...
from tree_cascade_noisyAND import tree_cascade_noisyAND
from tree_soft_cascade_noisyAND import tree_soft_cascade_noisyAND
...
for train_idxs, test_idxs in skf:
    ...
    # firm cascade:
    plambda = [0.19]
    AUC2, t3, accuracy3[t], F3[t], nnz = tree_cascade_noisyAND(trX, trY, teX, teY, trX1, teX1, trX2, teX2, w_h1, w_o,
                                                               b1, bo, plambda, a)
    time3[t] = t3 / 10000

    # soft cascade:
    beta = [0.0001*50]
    t2, accuracy2[t], F2[t], nnz_soft, AUC_soft = tree_soft_cascade_noisyAND(trX, trY, teX, teY, trX1, teX1, trX2, teX2,
                                                                             beta, K)
    time2[t] = t2/10000
...
plt.savefig('errorbar_accuracy_c5.pdf', bbox_inches='tight')
...
plt.grid(b='True', linestyle='--')
plt.savefig('errorbar_time_c5.pdf', bbox_inches='tight')
...
plt.ylim((0.75, 0.85))
plt.grid(b='True', linestyle='--')
plt.savefig('errorbar_fscore_c5.pdf', bbox_inches='tight')
